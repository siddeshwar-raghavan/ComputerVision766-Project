...
#for sartoEO flag-
#    labeldir   is immaterial - leave it as is
#    opticaldir - point to PAN folder - that way, the rotation will have happened in pretrain and avl in opticalproc dir
#mask files are allowed to be created; but not used
#do NOT use saronlytrain in settings.py
# remove transferoptical option in settings
# set larger epochs for sar training
...
# for sartoEO, no change is required except to switch off transferoptical flag
# also it is not required to create the inference_optical and inference_vector as well as output csv file; but we will let these files be created for simplicity

# one key matter is that the inference_continuous file is NOT rotated; so rotate and use it for next step where all optical PAN files will be run to get the mask
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sartoEO',
                        help='Tag for conversion of images')
